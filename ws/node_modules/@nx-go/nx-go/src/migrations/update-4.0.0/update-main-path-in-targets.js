...
async function update(tree) {
    ...
    for (const [projectName, projectConfig] of projects) {
        ...
        for (const [, target] of Object.entries(projectConfig.targets)) {
            const isNxGoBuildOrServe = target.executor === '@nx-go/nx-go:build' ||
                target.executor === '@nx-go/nx-go:serve';
            if (isNxGoBuildOrServe && target.options?.main) {
                const updatedMain = updateMainPath(target.options.main, projectConfig.root);
 ...
 */
function updateMainPath(mainPath, projectRoot) {
...
//# sourceMappingURL=update-main-path-in-targets.js.map